...
class env_tracker():
    def __init__(self, obs, verbose=False):
        ...
        self._init_log()
        self.mapping = {"px":0, "py":1, "vx":2, "vy":3}

    # Reinitialize log
    def _init_log(self):
        self.history = {"select":[], "params":[]}
    ...
    def update_log(self, attn_type ,x, y):
        ...
        if True: #self.intention is not None:
            self.history[attn_type].append([x,y])
            for tmp_x, tmp_y in self.history[attn_type]:
                self.log[attn_type][tmp_x,tmp_y]+=1
    ...
    def _cols_idx(self, val):
        ...
        if val in self.mapping:
            ...
            print("val not in mapping ",val, self.mapping)
            breakpoint()
    ...
    def select(self, params, criteria_str, value=None):
        ...
        if criteria_str.strip() in ["func_max", "func_min"]:
            idx_sorted = np.argsort(vehicles[:,col_idx])# 0:max -1:min
    ...
    def select_oblique(self, weight_str, condition_str):
        if self.verbose: print("[SELECT]", weight_str)
        vehicles = self.vehicles.copy()
        for params, col_idx in self.mapping.items():
            weight_str = weight_str.replace(str(params).strip(),"vehicles[:,%d]"%col_idx)

        weights = eval(weight_str) #(4,)
        select_w = eval("weights[weights%s]"%condition_str)
        if len(select_w)==0:
            self.obs_tmp[4:] *= 0
            return self.obs_tmp, None
        else:
            idx_sorted = np.argmax(select_w) if "<" in condition_str else np.argmin(select_w)
            vehicle_idx = np.argmin(abs(weights-select_w[idx_sorted]))
            self.vehicle_attn = vehicle_idx
        return self.obs_tmp, vehicle_idx
...
grammar = {
    ...
    "action": ["action='Left'", "action='Right'", "action='Faster'", "action='Slower'", "action='Idle'"],
    ...
    # oblique params
    "selection":["env.select_oblique('(<values>*px + <values>*py + <values>*vx + <values>*vy)','<comp_ops> <values>')"],
}
