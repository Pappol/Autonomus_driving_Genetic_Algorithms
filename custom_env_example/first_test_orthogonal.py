...
import glob
from grammar_dev import grammar, env_tracker
...
def fitness(decision_tree, data):
    ...
    reward_norm = {}
    #TODO: assume 5 class, fix later
    for i in range(5):
        reward_norm[i] = {"reward":[], "v_attn":[], "p_attn":[], "num":0, "env_reward":0}
    cm = {"y_pred":[], "y_true":[]}

    for step_idx in range(len(data)):
        env_step = data[step_idx] # keys: ['obs', 'grad', 'reward', 'action', 'q_values']; # env_step['grad'].keys() = [str(num)]
        ...
        if action_pred is None:
            continue
        reward_norm[action_pred]["num"]+=1
        reward_norm[action_pred]["env_reward"]+=env_step["reward"]

        # RL-AGENT INFORMATION
        # Q_values -> action_prob
        q_vals = np.array(env_step["q_values"]).flatten().tolist()
        action_prob = np.array([np.e**q_value for q_value in q_vals])
        action_prob = action_prob / np.sum(action_prob)

        # TRACKING INFO
        rl_grad = env_step["grad"][str(action_pred)] # key = str(num)
        rl_grad = np.array([np.e**x for x in rl_grad]).reshape(5,5)[:,1:]
        rl_grad_prob = np.round(rl_grad/np.sum(rl_grad),3)

        # DT INFORMATION
        dt_grad_select = pred_log["env"].log["select"].copy()
        dt_grad_params = pred_log["env"].log["params"].copy()

        
        # sub_function prediction
        # sub_pred = {} # dict of bool
        # for func_name in ["Left", "Right", "Faster", "Slower", "Idle"]:
        #     result = pred_log[func_name](func_name, pred_log["env"])
        #     sub_pred[func_name] = result
        # print(sub_pred)

        ## ======================== REWARD
        ## Policy probability with guided reward and grad bonus
        # GRAD BONUS: PARAMS ATTENTION [0;1]
        params_attn_bonus = np.sum(rl_grad_prob[dt_grad_params>0]) # Encorage use as much condition as possible
        reward_norm[action_pred]["p_attn"].append(params_attn_bonus*env_step["reward"]) 

        # GRAD BONUS: VEHICLE ATTENTION [0;1]
        rl_vehicle_attn = rl_grad_prob.sum(axis=-1)
        dt_vehicle_attn = dt_grad_select.sum(axis=-1)>0
        vehicle_attn_bonus = np.sum(rl_vehicle_attn[dt_vehicle_attn]) # Encorage use as much selection as possible
        reward_norm[action_pred]["v_attn"].append(vehicle_attn_bonus*env_step["reward"])

        p_attn_factor = max(params_attn_bonus, 0.01) # Reduce evolution pressure on operator 'get_params' 
        v_attn_factor = max(vehicle_attn_bonus, 0.01) # Reduce evolution pressure on operator 'select'
        
        # ACTION REWARD
        if action_pred == env_step["action"]:
            guided_reward = action_prob[action_pred]*env_step["reward"]
            reward_norm[action_pred]["reward"].append(guided_reward*p_attn_factor*v_attn_factor)
        else:
            # Opportunity reward = different_of_prob(<0) * potential_reward(fixed)
            # low_prob_difference -> lower punishment
            guided_reward = (action_prob[action_pred]-action_prob[env_step["action"]])*env_step["reward"]

            reward_norm[action_pred]["reward"].append(guided_reward)

        ## Q_value with opportunity cost
        # is_peak = True if action_pred == expected_action else False 
        # for action_idx, policy_prob in enumerate(action_prob):
        #     if not is_peak:
        #         sign = 1 if action_idx == action_pred else -1
        #         tmp_value = sign*policy_prob
        #     else:
        #         tmp_value = 1 if action_idx == action_pred else 0
        #     reward_norm[action_idx]["value"] += tmp_value 
        #     reward_norm[action_idx]["num"] += 1


        ## Q_value with bonus correct
        # if action_pred == expected_action:
        #     reward_norm[action]["value"] += 1
        # else:
        #     reward_norm[action]["value"] += action_prob[action] # < 1
        # reward_norm[action]["num"]+=1


        ## Q_value implementation
        # reward_norm[action]["value"]+=action_prob[action]
        # if action_pred == expected_action:
        #     reward_norm[action]["value"] += expected_reward
        # reward_norm[action]["num"]+=1

        cm["y_pred"].append(decision_tree.cvt_action(action_pred))
        cm["y_true"].append(decision_tree.cvt_action(env_step["action"]))

    for action_idx, reward_dict in reward_norm.items():
        for reward_name, val_list in reward_dict.items():
            if not isinstance(val_list, list):
                continue
            elif np.sum(val_list) == 0:
                reward_norm[action_idx][reward_name] = 0
            else:
                if reward_norm[action_idx]["env_reward"] == 0:
                    print(val_list, np.sum(val_list))
                    reward_norm[action_idx][reward_name] = np.round(np.mean(val_list),3)
                else:
                    reward_norm[action_idx][reward_name] = np.round(np.sum(val_list)/reward_norm[action_idx]["env_reward"],3)


    # dt_reward = [cls_reward["reward"]+cls_reward["v_attn"]+cls_reward["p_attn"] for cls_reward in reward_norm.values()]
    dt_reward = [cls_reward["reward"] for cls_reward in reward_norm.values()]
    fitness = np.mean(dt_reward)
    return fitness, cm, reward_norm
...
if __name__ == '__main__':
    ...
    class DataPreprocess:
        def __init__(self, path="data/*.json", num_sample=200, is_balance=True, num_cls=5):
            # breakpoint()
    ...
    # def fit_fcn(x, random_seed):
    #     return evaluate_fitness(fitness, random_seed, x)
